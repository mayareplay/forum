...
if(empty($gid) && empty($_G['member']['accessmasks']) && empty($showoldetails)) {
    ...
    if($catlist){
        foreach($catlist as $k=>$val){
            if(in_array($k,array(270,404,405,409,406,408,407))){
                unset($catlist[$k]);
            }
        }
    }
    ...
    $indexcontent = @file_get_contents(dirname(dirname(DISCUZ_ROOT)).'/data/cache/'.md5('wangjing_cn_bbs_index').'.dat');
	if($indexcontent){
		if($arr = unserialize($indexcontent)){
			$indexcontent = (string)$arr;
		}else{
			$indexcontent = '';
		}
	}
...
if(!$gid && (!defined('FORUM_INDEX_PAGE_MEMORY') || !FORUM_INDEX_PAGE_MEMORY)) {
    ...
    //增加今日发帖数
    $todayposts = intval($todayposts*2.1);
