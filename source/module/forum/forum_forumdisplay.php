...
if($filter !== 'hot') {
	...
	if($filterbool) {
        ...
        $filterarr2 = array(
            'inforum'=>array($_G['fid']),
            'displayorder'=>array(1),
            'sticky'=>4,
        );
        
        $threadlist = array_merge($threadlist ,C::t('forum_thread')->fetch_all_search($filterarr2, $tableid, $start_limit, $_G['tpp'], ''));
        unset($filterarr2);
        unset($filterarr1);
     	
       $_order = " $_GET[orderby] $_GET[ascdesc]";
       $filterarr['displayorder'] = array(0,1,2,3,4);
	  
    }
    
    if(!$filterbool && $stickycount){
        // array(0,1)
        $threadlist2 = C::t('forum_thread')->fetch_all_search($filterarr, $tableid, $start_limit, $_G['tpp'], $_order, '', $indexadd);
    }else{
        //array(0,1,2,3,4);
        // 首先查找置顶帖
        $filterarr2 = $filterarr;
        $filterarr2['displayorder']=array(1,2,3,4);
        $threadlist = C::t('forum_thread')->fetch_all_search($filterarr2, $tableid, $start_limit, $_G['tpp'], $_order, '', $indexadd);
        $_order = " $_GET[orderby] $_GET[ascdesc]";
	
        $threadlist2 = C::t('forum_thread')->fetch_all_search($filterarr, $tableid, $start_limit, $_G['tpp'], $_order, '', $indexadd);
       
    }
    for($i=0;$i<count($threadlist2);$i++){
        $threadlist2[$i]['displayorder'] = 0;
    }
	$threadlist = array_merge($threadlist, $threadlist2);
	unset($_order);
    unset($threadlist2);
