...
// 不显示贴内广告用户 需要修改相应查看贴子模板 去除$noads=1时候贴内广告

$noads = 0;
if(isset($_G['config']['ext_system']['noadsuid']) && in_array($thread['authorid'],$_G['config']['ext_system']['noadsuid'])){	
	$noads=1;
}
...
if(empty($_GET['viewpid'])) {
    ...
    $cache_dir = dirname(dirname(DISCUZ_ROOT)).'/data/cache/';
    $rightList = array();
    

    if(!IS_ROBOT && !checkmobile()){
     
     $top_pic = @file_get_contents($cache_dir.md5('top_baby_top_piclist').'.dat');
        if($top_pic){
    		if($arr = unserialize($top_pic)){
    			$top_pic = (string)$arr;
    		}else{
    			$top_pic = '';
    		}
        ...
        $bottom_thread_list = @file_get_contents($cache_dir.md5('ershoulist').'.dat');
    	if($bottom_thread_list){
    		if($arr = unserialize($bottom_thread_list)){
    			$bottom_thread_list = (string)$arr;
    		}else{
    			$bottom_thread_list = '';
    		}
    	}
        ...
        // 增加右边栏
        if(in_array($_G['fid'],array(358,106,362,361,502,341,369,382,505,383,421,424,450,363,364,447,452,368,422,350,379,464,310))){
            //亲子    
            $close_leftinfo = 1;
        
            DB::$db->connect(100);
            $query = DB::query("select n.*,s.cnname,s.logo,s.aboutthumb as thumb from yp_news n left join yp_shop s on n.shop_id=s.shop_id where n.shop_id>0 and n.displayorder>-1 and n.istop=1 and s.displayorder>-1 and s.status>-1 order by s.status desc,n.istop desc,n.dateline desc limit 10");
            ...
            while($row = DB::fetch($query)){
                ...
                $datalist[]=array(
                    'url'=>$infourl.'index.php?id='.$row['shop_id'].'&action=view&news_id='.$row['news_id'],
                    'subject'=>($row['title']),
            ...
            $rightList[]=array(
                    'name'=>'信息情报站',
                    'more'=>'http://info.wangjing.cn/index.php?action=news&type=huodong',
                    'type'=>'list',
                    'list'=>$datalist
            );
           ...
           // 最新报名
           $datalist = array();
           $query = DB::query("select j.*,n.title,s.cnname from yp_eventjoin j left join yp_news n on j.news_id=n.news_id  left join yp_shop s on j.shop_id = s.shop_id order by j.dateline desc limit 10");
            while($row = DB::fetch($query)){
                $datalist[]=array(
                    'url'=>'http://info.wangjing.cn/index.php?id='.$row['shop_id'].'&action=view&news_id='.$row['news_id'],
                    'subject'=>'<span style="color:#f26c4f">'.cutstr($row['u_name'],2,'*').'</span> 报名了 '. htmlspecialchars($row[cnname]).' '.$row['title']
        ...
        }else if($thread['fid'] == 256){
            $close_leftinfo=1; 
            //$right_column_conent = file_get_contents('');
            $filterarr = array();
            $filterarr['intype'] = $thread['typeid'];
            $filterarr['fid'] = $thread['fid'];
            $filterarr['inforum']=array($_G['fid']);
            $filterarr['displayorder']=array(0,1,2,3);
            $filterarr['sticky']=4;
            if($thread['typeid']){
                $threadlist = C::t('forum_thread')->fetch_all_search($filterarr, 0, 0, 10, 'dateline');
                for($i=0;$i<count($threadlist);$i++){
                    $threadlist[$i]['url'] = "thread-{$threadlist[$i][tid]}-1-1.html";
                }
                ...
                $rightList[]=array(
                    'name'=>$types[$thread['typeid']],
                    'more'=>'forum.php?mod=forumdisplay&fid='.$thread['fid'].'&filter=typeid&typeid='.$thread['typeid'],
                    'type'=>'list',
                    'list'=>$threadlist
                );
                
            }
        }else if($thread['fid']==312){
            $close_leftinfo=1;
        }
    ...
    //echo "<pre>";
	//print_r($_G['cache']['smilies']['searcharray']);
...
function viewthread_updateviews($tableid) {
	...
	// 取消搜索引擎和代理服务器的点击
	if($_SERVER['HTTP_REFERER'] && preg_match("/http:\/\/www\.wangjing\.cn/",$_SERVER['HTTP_REFERER'])){
        $views = rand(50,60);
    }else{
        $views = rand(5,10);
    }
	...
	if((!$_G['setting']['preventrefresh'] || $_G['cookie']['viewid'] != 'tid_'.$_G['tid']) && !$daili && !IS_ROBOT) {
		if(!$tableid && $_G['setting']['optimizeviews']) {
			if($_G['forum_thread']['addviews']) {
				if($_G['forum_thread']['addviews'] < 100) {
					//C::t('forum_threadaddviews')->update_by_tid($_G['tid']);
					DB::query('UPDATE %t SET `addviews`=`addviews`+%d WHERE tid=%d', array('forum_threadaddviews', $_G['tid'],$views));
				} else {
					if(!discuz_process::islocked('update_thread_view')) {
						$row = C::t('forum_threadaddviews')->fetch($_G['tid']);
						C::t('forum_threadaddviews')->update($_G['tid'], array('addviews' => 0));
						C::t('forum_thread')->increase($_G['tid'], array('views' => $row['addviews']+$views), true);
						discuz_process::unlock('update_thread_view');
					}
				}
			} else {
				C::t('forum_threadaddviews')->insert(array('tid' => $_G['tid'], 'addviews' => $views), false, true);
			}
		} else {
			//C::t('forum_thread')->increase($_G['tid'], array('views' => 1), true, $tableid);
			if(!$_G['uid']){
                if(rand(0,2) == 2){
                    C::t('forum_thread')->increase($_G['tid'], array('views' => $views), true, $tableid);
                }
            }else{
                C::t('forum_thread')->increase($_G['tid'], array('views' => $views), true, $tableid);
            }
