...
if($_GET['action'] == 'votepoll' && submitcheck('pollsubmit', 1)) {
	...
	if(submitcheck('activitysubmit')) {
        ...
        $verified = ($thread['authorid'] == $_G['uid'] || !$activity['ext_needverifiy']) ? 1 : 0;
        ...
        */
        ...
        if($activity['tipmsg']){
            if(!$_G['gp_message']){
                showmessage('请填写留言', NULL, array('max'=>$activity['tipmsg']), array('login' => 1));
            }
        }
        $ext_jingpaijiage =0;
        if($activity['ext_jingpaitype']){
            ...
            $neednum = intval($_GET['ext_jingpaijiage']);
            if($neednum<$activity['ext_jingpai']){
                 showmessage('您的出价不得低于起拍价格 '.$activity['ext_jingpai'], NULL, array('max'=>$activity['tipmsg']), array('login' => 1));   
            }
            if($activity['ext_jingpaitype'] == 1){
                // 威望
                $needcheckarr['extcredits1']= '-'.$neednum;
            }else{
                // 旺财
                $needcheckarr['extcredits2']= '-'.$neednum;
            }
            
            $ext_jingpaijiage = $neednum;
            checklowerlimit($needcheckarr);
        }
        
        
        
        $ext_friendnum = max(1,intval($_GET['ext_friendnum']));
        if($ext_friendnum > $activity['ext_firends']){
            showmessage('activity_maxcount', NULL, array('max'=>$activity['ext_firends']), array('login' => 1));
        }
        
        if($activity['number']){
           // 判断已报名人数是否已满
            if(($ext_friendnum+$activity['applynumber'])>$activity['number']){
                 showmessage('activity_maxlimit', NULL, array('max'=>($activity['number']-$activity['applynumber'])), array('login' => 1));   
            }
        }
        
        $needcheckarr = $usecredit = array();
        if((($_G['setting']['activitycredit'] && $activity['credit'])||$activity['ext_credit_2']) && empty($applyinfo['verified'])) {
            if($_G['setting']['activitycredit'] && $activity['credit']){
                // 扣除威望
                $neednum = $activity['credit']*$ext_friendnum;
                $ext_c = 'extcredits'.$_G['setting']['activitycredit'];
                $needcheckarr[$ext_c]='-'.$neednum;
                $usecredit[$_G['setting']['activitycredit']] = '-'.$neednum;
            }
            if($activity['ext_credit_2']){
            // 需要扣除旺财
                $neednum = $activity['ext_credit_2']*$ext_friendnum;
                $needcheckarr['extcredits2']= '-'.$neednum;
                $exkey = '2';
                $usecredit[$exkey] = '-'.$neednum;
            }
			checklowerlimit($needcheckarr);
            if($verified){
                // 直接审核则扣除
                updatemembercount($_G['uid'], $usecredit, true, 'ACC', $_G['tid']);
            }
		...
		if($applyinfo && $applyinfo['verified'] == 2) {
			$newinfo = array(
                ...
                'ext_friendnum'=>intval($_GET['ext_friendnum']),
                'ext_jingpaijiage'=>$ext_jingpaijiage,
			);
			C::t('forum_activityapply')->update($applyinfo['applyid'], $newinfo);
		} else {
			$data = array('tid' => $_G['tid'], 'username' => $_G['username'], 'uid' => $_G['uid'], 'message' => $message, 'verified' => $verified, 'dateline' => $_G['timestamp'], 'payment' => $payment, 'ufielddata' => $ufielddata,'ext_friendnum'=>$ext_friendnum,'ext_jingpaijiage'=>$ext_jingpaijiage);
	...
	*/
   	...
   	$isactivitymaster = $thread['authorid'] == $_G['uid'] || ($activity['ext_managers'] && $_G['uid'] && intval($activity['ext_managers'])== $_G['uid']) ||
						(in_array($_G['group']['radminid'], array(1, 2)));
		...
		if(empty($_GET['applyidarray'])) {
			...
			foreach($query as $row) {
				if($row['tid'] == $_G['tid']) {
                    ...
                    $_tmps[$row['uid']] = max(1,$row['ext_friendnum']);
				}
			}
			...
			} else if($_GET['operation'] == 'changeapply'){
			  // 设置成待审核
       	         
                 
                 C::t('forum_activityapply')->update_verified_for_thread(0, $_G['tid'], $_GET['applyidarray']);
                 $applynumber = C::t('forum_activityapply')->fetch_count_for_thread($_G['tid']);
				 C::t('forum_activity')->update($_G['tid'], array('applynumber' => $applynumber));
                
                 showmessage('待审核设置成功', "forum.php?mod=viewthread&tid=$_G[tid]&do=viewapplylist".($_GET['from'] ? '&from='.$_GET['from'] : ''), array(), array('showdialog' => 1, 'closetime' => true));
				...
				if($unverified) {
					...
					foreach($unverified as $uid) {
                       ...
                       // 扣除威望
					    $usecredit = array();
                        if($activity['credit'] || $activity['ext_credit_2']){
                            if($_G['setting']['activitycredit'] && $activity['credit']) {
          		                 $neednum = $activity['credit']*$_tmps[$uid];
                                 $usecredit[$_G['setting']['activitycredit']] = '-'.$neednum;
                            ...
                            if($activity['ext_credit_2']){
                                // 需要扣除旺财
                                $neednum = $activity['ext_credit_2']*$_tmps[$uid];
                                $exkey = '2';
                                $usecredit[$exkey] = '-'.$neednum;
                            }
                            
                            updatemembercount($uid, $usecredit, true, 'ACC', $_G['tid']);
                        }
	...
	*/
	...
	$applylist = array();
	$query = C::t('forum_activityapply')->fetch_all_for_thread($_G['tid'], 0, 2000, 0, 0);
