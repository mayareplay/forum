...
if(!in_array($view, array('hot', 'digest', 'new', 'my', 'newthread', 'sofa','quote'))) {
	$view = 'quote';
}
$lang = lang('forum/template');
$navtitle = $lang['guide_'.$view];
$perpage = 50;
...
if($view == 'quote'){
    if(!checkmobile()){
        dheader("Location: http://www.wangjing.cn");    
    }
    
    if($_G['cookie']['mobile'] == 'no'){
        header('location: '.'http://www.wangjing.cn');
        exit;
    }
    ...
    $theurl = 'forum.php?mod=guide&view=quote';
    if($_GET['show']!='huodong'){
        // 新闻
        // 论坛推荐
        $dataurl = 'wangjing_cn_bbs_touch_default';
    
        
        $file = dirname(dirname(DISCUZ_ROOT)).'/data/cache/'.md5($dataurl).'.dat';
        $count = 0;
        
        $cnt = @file_get_contents($file);
        $datalist = $replies =array();
        if($cnt){
    		if($arr = unserialize($cnt)){
    			$cnt = (string)$arr;
                if($cnt){
                    $datalist = json_decode($cnt,true);
                    $count = count($datalist);
                    $datalist = array_slice($datalist,($_G['page']-1)*$perpage,$perpage);
                    $tids = array();
                    for($i=0;$i<count($datalist);$i++){
                        $datalist[$i]['subject'] = mb_convert_encoding($datalist[$i]['subject'],'GB2312','UTF-8');
                        $datalist[$i]['desc'] = mb_convert_encoding($datalist[$i]['desc'],'GB2312','UTF-8');
                        $tids[]=$datalist[$i][tid];
                    }
                    if($tids){
                        $tidstr = dimplode($tids);
                        
                        $query = DB::query("select t.tid,t.author,t.replies,t.views,t.fid,f.name as forum from ".DB::table('forum_thread')." t left join ".DB::table('forum_forum')." f on t.fid=f.fid where t.tid in ($tidstr)");
                        while($row = DB::fetch($query)){
                            $replies[$row['tid']] = array(
                                'author'=>$row['author'],'replies'=>$row['replies'],'views'=>$row['views'],'forum'=>$row['forum'],
                            );
                        }
                   
                        for($i=0;$i<count($datalist);$i++){
                            $tid = $datalist[$i]['tid'];
                            $datalist[$i]['author'] = ($replies[$tid]?$replies[$tid]['author']:'');
                            $datalist[$i]['replies'] = ($replies[$tid]?$replies[$tid]['replies']:'0');
                            $datalist[$i]['forum'] = $replies[$tid]['forum'];
                            $datalist[$i]['views']= $replies[$tid]['views'];
                        }
                    }
                }
        ...
        $theurl = 'forum.php?view=quote&mod=guide&show=huodong&op='.$op;
        ...
        if($op =='tuijian'){
            // 推荐活动
            $query = DB::query("select a.tid,a.aid,t.subject,t.author,t.authorid,t.views,t.replies,f.name as forum from pre_forum_activity a left join pre_forum_thread t on a.tid=t.tid left join pre_common_member m on a.uid=m.uid left join pre_forum_forum f on t.fid=f.fid where (m.groupid in (1,2) or t.stamp=9) and t.displayorder>-1 order by t.dateline desc limit $start,$perpage");
            while($row = DB::fetch($query)){
                $datalist[]=$row;
                if($row['aid']){
                    $aids[]=$row['aid'];
            ...
            $query = DB::query("select a.tid,a.aid,t.subject,t.author,t.authorid,t.views,t.replies,f.name as forum from pre_forum_activity a left join pre_forum_thread t on a.tid=t.tid left join pre_common_member m on a.uid=m.uid left join pre_forum_forum f on t.fid=f.fid where f.fup=365 and t.displayorder>-1 order by t.dateline desc limit $start,$perpage");
            while($row = DB::fetch($query)){
            ...
            DB::$db->connect(100);
            $query = DB::query("select n.*,s.cnname,s.logo,s.aboutthumb as thumb from yp_news n left join yp_shop s on n.shop_id=s.shop_id where n.shop_id>0 and n.displayorder>-1 and n.istop=1 and s.displayorder>-1 and s.status>-1 order by s.status desc,n.istop desc,n.dateline desc limit $start,$perpage");
        
            $picurl = 'http://www.wangjing.cn/upload/ypshop/';
            $infourl = 'http://info.wangjing.cn/';
            while($row = DB::fetch($query)){
     
                if($row['thumb']){
                    $row['thumb'] = $picurl.$row['thumb'];
                }
                
                $datalist[]=array(
                    'url'=>$infourl.'index.php?id='.$row['shop_id'].'&action=view&news_id='.$row['news_id'].'&mobile=1',
                    'subject'=>dhtmlspecialchars($row['title']),
            ...
            $timelimit = time()-3600*24*60;
            $query = DB::query("select j.*,n.title,n.views,s.cnname,s.aboutthumb as thumb from yp_eventjoin j left join yp_news n on j.news_id=n.news_id left join yp_shop s on j.shop_id=s.shop_id  where j.uid='$_G[uid]' and j.dateline >$timelimit group by j.news_id order by j.dateline ");
            $list = array();
            while($row = DB::fetch($query)){
                $list[$row['dateline']]=array(
                    'subject'=>$row['title'],
                    'url'=> $infourl.'index.php?id='.$row['shop_id'].'&action=view&news_id='.$row['news_id'].'&mobile=1',
                    'thumb'=>$row['thumb']?$picurl.$row['thumb']:'',
                    'forum'=>$row['cnname'],
                    'views'=>$row['views'],
                    'dateline'=>$row['dateline']
                );
            ...
            DB::$db->connect(1);
            $query = DB::query("select tid from pre_forum_activityapply where uid='$_G[uid]' order by dateline desc limit 50");
            $tids = array();
            while($row=DB::fetch($query)){
                $tids[]=$row['tid'];
            }
            ...
            if($tids){
                $tidstr = dimplode($tids);
                $query = DB::query("select a.tid,a.aid,t.subject,t.author,t.dateline,t.authorid,t.views,t.replies,f.name as forum from pre_forum_activity a left join pre_forum_thread t on a.tid=t.tid left join pre_common_member m on a.uid=m.uid left join pre_forum_forum f on t.fid=f.fid where t.tid in ($tidstr) and t.displayorder>-1 order by t.dateline desc limit $start,$perpage");
                while($row = DB::fetch($query)){
                   // $datalist[]=$row;
                    if($row['aid']){
                        $aids[]=$row['aid'];
                    }
                    $list[$row['dateline']] = $row;
                }
            }
        ...
        //$query = DB::query("select t.tid,t.author,t.replies,t.views,t.fid,f,name from ".DB::table('forum_thread')." t left join ".DB::table('forum_forum')." f on t.fid=f.fid left join pre_forum_activity a on t.tid=a.tid where where t.special=4");

        

        if($aids){
            $idstr = dimplode($aids);
            $query = DB::query("select aid,tableid from pre_forum_attachment where aid in ($idstr)");
            $_aid=  array();
            while($row = DB::fetch($query)){
                $_aid[$row['tableid']][]=$row['aid'];
            }
  
            if($_aid){
                 $atts = array();
                foreach($_aid as $tableid =>$ids){
                    $_s = dimplode($ids);
                    $query = DB::query("select aid,attachment from pre_forum_attachment_".$tableid." where aid in ($_s)");
                   
                    while($row = DB::fetch($query)){
                        $atts[$row['aid']] = $row['attachment'];
                    }
                }
            }
                ...
                for($i=0;$i<count($datalist);$i++){
                    if($datalist[$i]['aid']){
                        $datalist[$i]['thumb'] = 'data/attachment/forum/'.$atts[$datalist[$i]['aid']];
                    }
                }
    ...
    $multipage = multi($count, $perpage, $_G['page'], $theurl, $_G['setting']['threadmaxpages']);
}elseif($view != 'index') {
