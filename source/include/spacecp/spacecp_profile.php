...
// 增加phone
$operation = in_array($_GET['op'], array('base', 'contact', 'edu', 'work', 'info', 'password', 'verify','phone')) ? trim($_GET['op']) : $defaultop;
...
if($operation == 'phone'){
    ...
    //print_r($_G);
    /*
    include ("./source/phonebind.class.php");
    $bind = new phonebind($_G['uid'],$interval);
    $binddetial = $bind->getBindDetial();
    */
    if($_GET['d']== 'sendcode' && $_GET['mobile']){
       // include_once('source/sms.class.php');
        
        //$result =  $_GET['mobile'];


        //'验证码:'.$rand;
        $res = $bind->sendCode($_G['uid'],$_GET['mobile']);
        
        if(!$res){
              //$result = DSendMessage::sendMsg($mobile,$rand);
            include template('common/header_ajax');
            //echo '<script>alert("eee")</script>';
            echo $bind->errors[$bind->error];
        ...
        //$result = DSendMessage::sendMsg($_GET['mobile'],'验证码为:'.$res);
        
        
        $result = sms::sendMsg($_GET['mobile'],'激活码为:'.$res);
        include template('common/header_ajax');
        echo '';
        include template('common/footer_ajax');
    ...
    if($binddetial){
        $nomark = substr($binddetial['mobile'],0,3).'*****'.substr($binddetial['mobile'],9,11);
    }
    ...
    if(submitcheck('profilesubmit')) {
       ...
       $res = $bind->bind_phone($_G['uid'],$_POST['mobile'],$_POST['code']);
       ...
       if($res){
            ...
            profile_showsuccess('绑定成功'.$addmsg);
       }else{
        if($bind->error == -1 || $bind->error == -4){
            profile_showerror('mobile',$bind->errors[$bind->error]);
            }else{
                 profile_showerror('code',$bind->errors[$bind->error]);
            }
       }

    }else if(submitcheck('unbindsubmit')){
        // 解除绑定
        
        if(!$binddetial){
             //profile_showsuccess('您已经解除绑定');
             profile_showerror('mobileold','您已经解除绑定');
        }
        
        if(!$_POST['mobileold']){
             profile_showerror('mobileold','请填写原电话号码');
        }
        
        
       $res = $bind->unbind_phone($_G['uid'],$_POST['mobileold']);
       if($res){
            profile_showsuccess('解除绑定成功');
       }else{
            if($bind->error == -1 || $bind->error == -4){
                 profile_showerror('mobile',$bind->errors[$bind->error]);
            }else if($bind->error == -7||$bind->error==-8){
                 profile_showerror('mobileold',$bind->errors[$bind->error]);
            }else{
                profile_showerror('code',$bind->errors[$bind->error]);
            }
    ...
    }else if(submitcheck('modbindsubmit')){
        // 修改绑定
        if(!$_POST['mobileold']){
             profile_showerror('mobileold','请填写原电话号码');
        }
        
        $res = $bind->modBind($_G[uid],$_POST['mobile'],$_POST['code'],$_POST['mobileold']);
        
        if($res){
            profile_showsuccess('修改绑定成功');
        }else{
       //profile_showerror('mobile',$bind->errors[$bind->error]);
            if($bind->error == -1 || $bind->error == -4){
                 profile_showerror('mobile',$bind->errors[$bind->error]);
...
if($operation == 'password') {
	...
	if($_GET['resend'] && $resend) {
        ...
        if($_G['groupid']==8){
            ...
            $regip = $_G['clientip'];
            $city = convertip($regip);
            if(preg_match('/北京/',$city)){
                 emailcheck_send($space['uid'], $toemail);
            }else{
           	     showmessage('非北京用户只能手机验证', "active.php?by=mobile");
            }
        }else{
             emailcheck_send($space['uid'], $toemail);
        }
        ...
        if($_GET['from']=='active'){
            	showmessage('send_activate_mail_succeed', "active.php?by=email");
        }
