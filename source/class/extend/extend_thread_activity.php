...
class extend_thread_activity extends extend_thread_base {
	...
	public function before_newthread($parameters) {
        ...
        // 活动扩展 start.....
        $this->activity['ext_firends'] = max(intval($_GET['ext_firends']),1);
        $this->activity['ext_needverifiy'] = intval($_GET['ext_needverifiy'])==1?1:0;
		$this->activity['tipmsg']=trim($_GET['tipmsg']);


		if(in_array($this->member['groupid'],array(1,2))){
			// 属于管理员
       
            $this->activity['ext_credit_2'] = intval($_GET['ext_credit_2']);
            $this->activity['ext_jingpaitype'] = intval($_GET['ext_jingpaitype']);
            $this->activity['ext_jingpai'] = intval($_GET['ext_jingpai']);
            $this->activity['ext_jiangnum'] = intval($_GET['ext_jiangnum']);
            $this->activity['ext_tuangou'] = intval($_GET['ext_tuangou']);
            $this->activity['ext_managers'] = intval($_GET['ext_managers']);
		}
	...
	public function after_newthread() {
		if($this->group['allowpostactivity']) {
			$data = array('tid' => $this->tid, 'uid' => $this->member['uid'], 'cost' => $this->activity['cost'], 'starttimefrom' => $this->activity['starttimefrom'], 'starttimeto' => $this->activity['starttimeto'], 'place' => $this->activity['place'], 'class' => $this->activity['class'], 'gender' => $this->activity['gender'], 'number' => $this->activity['number'], 'expiration' => $this->activity['expiration'], 'aid' => $_GET['activityaid'], 'ufield' => $this->activity['ufield'], 'credit' => $this->activity['credit']);
			 // 增加扩展数据 =========
            $_extdata = array(
                'ext_firends'=>$this->activity['ext_firends'],
                'ext_needverifiy'=>$this->activity['ext_needverifiy'],
                'tipmsg'=>$this->activity['tipmsg'],
            );
            
            if(in_array($this->member['groupid'],array(1,2))){
                $_extdata = array_merge($_extdata,
                    array(
                        'credit' => $this->activity['credit'],
                        'ext_credit_2'=>$this->activity['ext_credit_2'],
                        'ext_jingpaitype'=>$this->activity['ext_jingpaitype'],
                        'ext_jingpai'=>$this->activity['ext_jingpai'],
                        'ext_jiangnum'=>$this->activity['ext_jiangnum'],
                        'ext_tuangou'=>$this->activity['ext_tuangou'],
                        'ext_managers'=>$this->activity['ext_managers']
                    )
                );
            }
            
            // 增加扩展数据 =========
            $data = array_merge($data,$_extdata);
	...
	public function before_editpost($parameters) {
		...
		if($isfirstpost) {
			if($this->thread['special'] == 4 && $this->group['allowpostactivity']) {
                ...
                //-------------------------
                $activity['ext_firends'] = max(intval($_GET['ext_firends']),1);
                $activity['ext_needverifiy'] = intval($_GET['ext_needverifiy'])==1?1:0;
				$activity['ext_credit_2'] = intval($_GET['ext_credit_2']);
				$activity['tipmsg'] =trim($_GET['tipmsg']);
                $activity['ext_jingpaitype'] = intval($_GET['ext_jingpaitype']);
                $activity['ext_jingpai'] = intval($_GET['ext_jingpai']);
                $activity['ext_jiangnum'] = intval($_GET['ext_jiangnum']);
                $activity['ext_tuangou'] = intval($_GET['ext_tuangou']);
                $activity['ext_managers'] = intval($_GET['ext_managers']);
				//-------------------------
                ...
                // 添加活动扩展
                $_extdata = array(
                    'ext_firends'=>$activity['ext_firends'],
                    'ext_needverifiy'=>$activity['ext_needverifiy'],
                    'ext_credit_2'=>$activity['ext_credit_2'],
                    'tipmsg'=>$activity['tipmsg'],
                    'ext_jingpaitype'=>$activity['ext_jingpaitype'],
                    'ext_jingpai'=>$activity['ext_jingpai'],
                    'ext_jiangnum'=>$activity['ext_jiangnum'],
                    'ext_tuangou'=>$activity['ext_tuangou'],
                    'ext_managers'=>$activity['ext_managers'],
                );
